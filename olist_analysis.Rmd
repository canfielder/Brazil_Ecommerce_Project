...
## Convert Time
The dataset has a variety of date-time stamp variables. Some of these will be an important variable for further analysis. We need to convert date-time values, which were imported as characters, into Posix date-time values. There are date-time variables in the order table and review table. 
...
### Review Table
After performing the transformation for the review table, I notice that the review creation date does not come with a useful time stamp, only a date. 
```{r}
# Define Columns which need to converted from character to date-time.
...
df_cust_seg_6 <- df_cust_seg_5 %>% 
  ...
  select(-(review_id:review_answer_timestamp))

df_cust_seg_6 %>% glimpse()
```
...
```{r}
df_cust_seg_7 <- df_cust_seg_6 %>% 
  left_join(df_order_items_agg_2, by = "order_id")

df_cust_seg_7 %>% glimpse()
```
...
df_cust_seg_8 <- df_cust_seg_7 %>% 
  left_join(df_order_pay %>% 
              select(order_id,
                     payment_installments), 
            by = "order_id")

df_cust_seg_8 %>% glimpse()
```

### Aggregate By Customer
Now that we have all the raw data on a order level, we need to aggregate the data for the customer. For this, we are going to calculate the average for every variable. We can also now drop order_id, as it is no longer needed.

```{r}
df_cust_seg_9 <- df_cust_seg_8 %>% 
  select(-order_id) %>% 
